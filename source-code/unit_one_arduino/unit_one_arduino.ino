...
#define BLYNK_AUTH_TOKEN "CR-UnBGsw6kLsrWJ_f3VMelCg3O56YIR"
...
void loop() {
  ...
  in_voltage_battery = abs((adc_voltage_battery / (R2/(R1+R2))) - 1); // RUMUS SEBENARNYA HARUSNYA GA ADA -1, DIHARUSKAN KRN ETHERNET SHIELD
  ...
  in_voltage_solar = abs((adc_voltage_solar / (R2/(R1+R2))) - 1); 
  teganganPanel = int(in_voltage_solar);

  if (resultLDRsensor) {
      Serial.println("Darkness detected");
      digitalWrite(relay3, LOW);              // RELAY 2 HIDUP
  }
  else {
      Serial.println("Light detected");
      digitalWrite(relay3, HIGH);             // RELAY 2 MATI
  }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,topic2) == 0){ 
    ...
    if((var1.toInt() < 11 || var2.toInt() < 11) && teganganBaterai > 11) {
        digitalWrite(relay1, LOW);    // RELAY 1 HIDUP
        Serial.println("Relay 1 hidup untuk PJU 2");
    } 
    else {
      digitalWrite(relay1, HIGH);     // RELAY 1 MATI
      Serial.println("Relay 1 mati untuk PJU 2");
    }
  }
  ...
  if(strcmp(topic,topic3) == 0){ 
    ...
    if((var1.toInt() < 11 || var2.toInt() < 11) && teganganPanel > 11) {
        digitalWrite(relay1, LOW);    // RELAY 1 HIDUP
        Serial.println("Relay 1 hidup untuk PJU 3");
    }
    else {
      digitalWrite(relay1, HIGH);     // RELAY 1 MATI
      Serial.println("Relay 1 mati untuk PJU 3");
    }
  } 
